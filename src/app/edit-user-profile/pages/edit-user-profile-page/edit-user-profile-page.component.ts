...
@Component({
    ...
    imports: [AsyncPipe, ReactiveFormsModule, MatButtonModule, MatCardModule, MatFormFieldModule, MatInputModule, LocationTypeaheadComponent, AccessibleLocationsTableComponent],
...
export class EditUserProfilePageComponent implements OnInit {
    ...
    readonly form = this.fb.nonNullable.group({
        ...
        primaryLocationSearch: this.fb.control<string | null>(null),
        accessibleLocationSearch: this.fb.control<string | null>(null)
    });
    ...
    ngOnInit(): void {
        ...
        this.state$.pipe(filter((s) => !!s.user), take(1)).subscribe((s) => this.form.patchValue({
            firstName: s.user!.firstName,
            lastName: s.user!.lastName,
            email: s.user!.email,
            primaryLocationId: s.user!.primaryLocationId,
            primaryLocationSearch: s.user!.primaryLocationName
        }));
    }
    ...
    saveAndExit(): void {
        ...
        const request: UpdateUserProfileRequest = {
            ...
            primaryLocationId: this.form.controls.primaryLocationId.value
        };
        this.facade.saveProfile(request).subscribe(() => void this.router.navigateByUrl('/'));
    }
